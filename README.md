...
- 最新本地安装包：`D:\Agent\app\releases\Echo-v1.7.8.apk`
...
- 对话页标题恢复更大的字号，并在顶部胶囊工具栏内垂直居中。
- 模型最大上下文识别改为动态读取模型列表元数据，未知模型默认按 1M 处理。
- 上下文超限时会自动缩小窗口并重组上下文重试，减少直接报错。
- 模型回复底部的思考 token 显示补充 `tokens` 单位。
- 模型响应耗时移入蓝色思考过程气泡，显示在“思考过程”右侧。
...
```text
D:\Agent\app\releases\Echo-v1.7.8.apk
```
