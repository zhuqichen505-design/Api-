...
## 本次用户可见更新 (v1.9.25)
- 对话页顶部悬浮胶囊栏液态玻璃重构：去除外层底色，消除多余矩形边框与背景模糊报错。
- 屏幕顶部浮动报错卡片毛玻璃化：移除粉红实色背景，升级为统一半透明液态玻璃卡片。
- 对话输入栏右上角弧线手柄：绘制四分之一同心圆弧手柄 `⌒`，支持垂直拖动自由调高（42dp~360dp），点击切换最小/最大高度，新对话默认最小高度。
- 流式输出跟手自动滚动：处于底部时实时吸附下滚，用户上滑阅读时自动暂停，滑回底部自动恢复跟随。
- 报错气泡默认折叠：AI 回复错误时默认折叠详细报错信息仅显示关键摘要，支持双击快速展开/收回。
- 主动停止生成防二次弹窗：用户主动点击停止时静默拦截底层网络关闭引发的二次报错弹窗与气泡。
- 新建对话默认最大 Token 严格固定为 50,000，思考模式默认开启，联网搜索默认关闭。
- 新建会话内专属记忆默认关闭，同时优化专属记忆高信噪比提取正则规则。
- 设置页 API 配置顶部新增“新对话默认 API”卡片，支持一键切换。
- 模型连接中与思考中文案支持自定义模板配置，支持 `{model}` 占位符并提供一键重置默认值。
- 思考胶囊支持双击切换展开/折叠，历史消息进入会话时默认折叠思考过程。
- 划选文本浮动工具栏防重绘闪烁，补齐剪切与粘贴操作按钮。
- 右侧滚动条手势重构为稳定事件流，彻底解决长列表滑动断触。
- 设置页更新说明纯净化，仅保留 v1.9.25 核心升级亮点。
