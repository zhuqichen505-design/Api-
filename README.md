...
## 本次用户可见更新 (v1.9.27)
- **对话页顶部悬浮工具栏和错误提示无边缘包裹且全屏穿透**：彻底移除 Scaffold 顶层 topBar 占位，消息列表与背景全屏贯通铺满屏幕顶部并纳入毛玻璃取样；悬浮栏完全复用输入框玻璃规范，文字滚动至顶部优雅透过模糊。
- **输入框放大弧线手柄大小彻底统一**：手柄圆角常数永久固定为拖动后的精致小规格（22dp/17dp），拖拽前后大小完全恒定一致。
- **对话页辅助滑动 4 个按键变浅且半透明透光化**：4 个快捷滑动键（到顶、上一条输入、下一条输入、到底）全面从深色实色重构为柔和浅天蓝阶梯半透明配色，搭配 1dp 白色微晕描边与 1dp 轻阴影。
- **长按文本选区弹出复制工具栏高频闪烁与复制失效彻底根治**：文本工具栏状态与 Compose 快照深度解耦，就地复用菜单实例并引入 16px 抖动过滤阈值，彻底阻断每秒数十次重组的闪烁死循环，复制与引用秒显且 100% 稳如磐石。
