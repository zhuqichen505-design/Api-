...
- 全局统一液态玻璃 panel/control/input/bubble token，减少浅色方块、泛白遮罩和滚动变色。
- 对话页顶部栏、输入栏、用户气泡、AI 气泡、思考块、附件预览和智能搜索开关改为稳定玻璃材质。
- 首页对话胶囊卡片改为液态玻璃，文件夹筛选、新对话入口和 Echo 艺术字使用统一设计语言。
- 对话设置“最大 Token 数”改为数字输入 + tokens 单位块，避免窄屏错位。
- 使用统计模型明细改为带表头和右对齐数值的半透明表格。
- 设置、历史记录、文件夹管理和备份相关卡片迁移到统一玻璃 token，深色模式文字对比度更稳定。
