...
## 本次用户可见更新 (v1.9.28)
- **输入框半透明液态毛玻璃效果完美还原**：在全屏根节点恢复全屏全域毛玻璃取样源与背景图，打破原先在 Scaffold 内部放置导致的底部栏无背景、无取样源的缺陷，彻底解决输入框失去半透明、沦为纯色/黑底的问题。
- **顶部悬浮工具栏与错误提示真实毛玻璃半透明透字**：彻底消除 `echoHazePanel` 内部的双层背景叠加，全屏贯通的消息列表向上滚动时平滑穿透悬浮栏底层，文字与气泡实时被模糊并半透明隐约透出。
- **模型思考胶囊文字无法显示问题彻底根除**：移除思考胶囊内部导致无限约束冲突与测量裁剪的 `horizontalScroll` 容器，改为标准 `Text` 搭配 `weight(1f, fill = false)` 与 `TextOverflow.Ellipsis`，消除宽度为 0 与滚动偏移溢出，多级文案严格兜底，100% 稳定清晰呈现。
- **对话页面气泡残留彻底修复**：分支生成组 ID 在生成结束时自动重置为 null，切换会话重置分支状态，过滤无效纯空异常消息，杜绝幽灵气泡残留。
