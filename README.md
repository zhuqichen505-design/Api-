...
## 本次用户可见更新 (v1.9.26)
- **顶部悬浮栏与报错弹窗直接复用输入框玻璃样式**：彻底解决双层背景与状态栏背景穿透异常，界面风格纯净统一。
- **输入框右上角弧线控制手柄同心贴合**：根据当前圆角严格计算同心圆弧半径（收起 30dp，展开 22dp），颜色一致略深，拖拽微调随手自如。
- **大模型非标颜色标签与尾随星号容错解析**：十六进制容错映射器自动纠错变体（如 `2B7DEP` 纠错为天蓝 `#2B7DE0`），自动消费净化标签闭合后的悬挂尾随星号（如 `</font>*`）。
- **用户输入气泡与上一回复纵向间距加宽**：用户气泡距上一条 AI 输出增加 18dp 额外呼吸间距，视觉阅读感大幅提升。
- **思考强度快速档纯正柔和天蓝配色**：全应用思考强度快速档（Low/Fast）代表色全面升级为雅致纯正的柔和天蓝 `#60A5FA`。
- **对话页悬浮滚动快捷键升级为4键独立体系**：升级为到顶（双线）、上一条输入（单线）、下一条输入（单线）、到底（双线），按思考强度阶梯渐进色彩，智能寻址就近跳转。
- **长按划选弹窗防闪烁与复制失效彻底修复**：基于能力标志与矩形误差防抖判等，杜绝 Compose SelectionManager 测量高频重建 Popup 导致的 60fps 闪烁死循环，使长按划选后的复制与引用秒级稳定响应。
