...
- 最新本地安装包：`D:\Agent\app\releases\Echo-v1.7.13.apk`
...
- 对话设置弹窗、上下文弹窗、备份恢复/删除确认弹窗统一为圆润玻璃风格。
- 历史记录页、文件夹管理页、使用统计页统一使用首页壁纸与玻璃卡片体系。
- 使用统计优化 token 读取准确度，避免把无法拆分的 token 全部误算为输出，并在图表中单独显示“其他”。
- 设置系统提示词时点击输入框不再让对话设置列表自动回到顶部。
- 对话页标题字号进一步减小。
- 流式输出只有当前页面处于底部时才自动跟随；用户手动上滑阅读时不再强制拉到底部。
- 移除独立“生成中”标志，流式输出三点改为淡蓝色。
...
```text
D:\Agent\app\releases\Echo-v1.7.13.apk
```
