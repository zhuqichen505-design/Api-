...
- 最新本地安装包：`D:\Agent\app\releases\Echo-v1.7.9.apk`
...
- Kotlin 升级到 2.x 构建链，并切换到 Kotlin Compose Compiler Gradle 插件。
- Compose 运行库 BOM 从 `2023.10.01` 更新到 `2024.06.00`，移除旧的手动 Compose 编译器扩展版本配置。
- AGP 升级到 `8.10.1`、Gradle Wrapper 升级到 `8.11.1`，Room 升级到 `2.8.4` 以适配 Kotlin 2.x 注解处理。
- 首页搜索栏、创建对话按钮、对话页顶部工具栏和输入框升级为液态玻璃效果。
- 玻璃控件加入真实背景模糊、渐变高光、边缘亮线和轻微阴影，不再只是灰色边框。
...
```text
D:\Agent\app\releases\Echo-v1.7.9.apk
```
