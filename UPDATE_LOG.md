...
### 1. 核心需求落实与技术重构详情
1. **滚动摘要功能彻底删除，根治时间点倒流与剧情原地踏步（用户核心痛点终结）**：
   - **根本原因剖析**：
     - 用户现场反馈：滚动摘要总结最近对话后不会实时更新，且模型始终停留在滚动摘要总结的最后时间点，导致后续无论发送什么、推进多久对话，模型都原地踏步；
     - 深度审查发现两大核心病灶：
       1. `buildEffectiveSystemPrompt` 中无论在常规模式还是角色扮演模式下，均硬性拼接了 `<session_summary>` 及 `【前序剧情滚动摘要】`。摘要中固化的“当前停顿时间点”强制作为系统级最高先验事实注入，直接将模型锁死在过去的陈旧时间点上，彻底丧失推进未来剧情的能力；
       2. 旧版 `buildContextBundle` 与 `buildContextUsageSnapshot` 中只要存在滚动摘要，就暴力执行 `candidateMessages = usableMessages.filter { it.id > summarizedThrough }`，将上次摘要前序的所有对话物理切断抹除，导致模型直接丢失前几段关键对话上下文，无法掌握剧情来龙去脉。
   - **全面重构落地**：
     - **彻底移除滚动摘要拼接注入**：重构 `AiRepository.buildEffectiveSystemPrompt`，全面废除 `summaryBlock`（`<session_summary>`）与角色扮演模式下的 `【前序剧情滚动摘要】` 拼接，系统提示词彻底解绑陈旧时间点对剧情推进的锁死；
     - **彻底废除 `summarizedThrough` 物理切断**：在 `buildContextBundle` 与 `buildContextUsageSnapshot` 中彻底废除针对摘要水线的物理过滤逻辑，所有历史消息直接依据 Token 预算与近期保护策略进行安全装填，`summary` 字段统一安全返回 `null`；
     - **废除隐式后台长文本滚动摘要生成**：将 `ensureRollingSummary` 重构为直接安全返回 `null`，不再在后台静默产生过时长摘要。
2. **最近十几次对话（16+ 条）无条件无损保留**：
   - 在 `AiRepository.kt` 中确立硬性常量 `UNCOMPRESSED_RECENT_MESSAGE_COUNT = 16`（涵盖至少 8~10 轮高质量完整对话）；
   - 在 `buildContextBundle`、`compressConversationContext` 与 `buildContextUsageSnapshot` 中全面贯彻该保护门限：
     - 对话轮次处于最近 16 条消息窗口内时，绝不执行任何丢弃或压缩；
     - 上下文装填在倒序扫描时，对最近 16 条对话实施最高优先级保留（即使超出一般预算也优先保全这 16 条活跃消息原文），让模型能够原汁原味完整读取最近的对话与细节，自然丝滑地推进后续剧情。
3. **压缩上下文全面转向“梳理时间线”与“会话专属记忆提炼”**：
   - 彻底将“会话记忆”和“时间线系统”作为过往对话的压缩成果：
     - 当上下文需要优化且超出最近 16 条活跃消息时，通过 `compressConversationContext` 调用 `fallbackLocalTimelineScan` 将较早历史梳理沉淀为时序单调严密的 `TimelineNode`；
     - 同时提取关键约束与未决待办，直接去重沉淀为专属的会话记忆 `MemoryItem`；
     - 将 `rollingSummary` 彻底置为 `null`，仅保留裁剪水线标记 `messageId` 与时间戳；
     - `generateRollingSummaryNow` 亦无缝重定向至时间线梳理与记忆提炼引擎，保持接口向下兼容。
4. **上下文用量与压缩界面全面焕新 (UI/UX 升级)**：
   - 重构 `ChatContextComponents.kt`：
     - 顶部副标题更新为“模型窗口预算、时间线梳理与上下文压缩管理”；
     - 卡片 1 由“滚动摘要管理”全面升级为“时间线与会话记忆 (Timeline & Memory)”，操作按钮更新为“梳理时间线与提炼记忆”；
     - 用量详情列表中移除滚动摘要行，新增“较早历史消息”、“时间线/记忆成果”与“压缩沉淀水线”；
     - 状态指示卡片文案同步更新为“可按需梳理时间线与提炼记忆”、“最近十几次历史对话完整保留”；
...
### 1. 核心需求落实与技术重构详情
1. **滚动摘要标点断句保护彻底根除暴力截断（核心痛点根治）**：
   - **根本原因排查**：
     - **超时静默降级假成功**：`generateRollingSummaryNow` 超时仅 25s，后台触发仅 15s，在数十条长历史请求现代思考模型时极易超时；超时后静默降级为本地抽取式兜底并提示“提炼成功”，让用户误以为是 AI 生成结果；
     - **硬编码字符强切腰斩**：`AdvancedMemoryEngine.generateExtractiveStructuredSummary` 中存在多处无视标点语意的字符物理强切（如 `take(70)`、`clean.take(100)`），直接在汉字/单词中央强行腰斩，导致句子碎裂、表达残缺；
     - **预算收缩缺乏断句保护**：`compactTextToTokenBudget` 压缩超长摘要时同样采用直接字符切断，抹杀句末标点。
   - **全面修复落地**：
     - 在 `AdvancedMemoryEngine.kt` 中设计并实现 `extractCompleteSentence(text, maxChars)`，严格依据句末主标点（`。`、`！`、`？`、`\n`、`；`、`.`、`!`、`?`）或子句标点（`，`、`、`、`,`）安全闭合，彻底淘汰任何 `take(70)` 等物理截断逻辑；
     - 在 `compactTextToTokenBudget` 中引入整句边界保护，任何预算收敛都确保整句闭合。
2. **滚动摘要总结内容去机械化与上下文深度提炼（质量跃升）**：
   - **根本原因排查**：
     - 兜底提取在无明确决策词时盲目摘抄最后几条消息的前 70 字拼凑成伪里程碑，并冠以“下面是较早对话的高保真结构化上下文状态机...”等生硬术语；
     - Prompt 中强制要求输出“多维滚动上下文状态机”及“时空演变与关键时间节点（起始时间、总跨度、当前故事停顿节点，严禁时序倒流或模糊为昨天）”，迫使大模型在日常工作、编程、学术等普通对话中强行脑补编造虚构小说时间线，产生荒谬、空洞的机械废话。
   - **全面修复落地**：
     - 彻底清理生硬的“上下文状态机”等开发术语，提取关键轮次具有真实信息量的完整整句；
     - 重构 `buildStructuredSummaryPrompt`，消除对普通对话强行编造小说剧情的副作用，区分常规讨论与剧情演变，重点提炼核心主题、关键共识与决策、当前进展与未决事项；
     - 明确指示大模型：“语言必须自然通顺、表述完整，每句话都必须意思表达充分，严禁被暴力截断或半句截断”。
3. **滚动摘要接入 600 秒专用长分析通道（高可用升级）**：
   - 将 `generateRollingSummary` 升级至 `RetrofitClient.getAnalysisService`（拥有 600 秒超长超时、专用连接池与心跳保活）；
   - 补全 `normalizeApiBaseUrl` 与 `parseApiKeys` 多 Key 轮询容灾；
   - 深度思考模型支持：兼容 `choice.message.reasoning_content` 内容提取；
   - 支持非流式被拒时的流式保底通道（`executeStreamingCompletion`）；
...
### 1. 核心需求落实与技术重构详情
1. **对话时间线自动识别用户确认机制（需求 1）**：
   - **痛点与风险**：对话中模型自动识别到时序或事件变动时，若直接静默入库，用户无法干预或撤销，一旦识别有偏差就会污染剧情时间线。
   - **重构方案**：重构 `evaluateAndAutoUpdateTimeline` 为无副作用检测，变动结果封装为 `AutoTimelineUpdateResult`；在 `ChatViewModel` 中引入 `pendingTimelineProposal` 状态流；在 `ChatScreen` 顶部以 `EchoGlassCard` 呈现待确认卡片，展示推进时空与事件详情，提供【确认应用】与【忽略】操作，用户点击【确认应用】后才原子写入数据库。
2. **多轮对话长事件修改补充与合并能力（需求 2）**：
   - **痛点与风险**：一件由多轮对话组成的复杂事件被识别多次，模型只会追加事件（`APPEND`），无法识别已有事件并进行深化修改（`UPDATE`），导致时间线记录大量碎片化流水账。
   - **重构方案**：在 `evaluateAndAutoUpdateTimeline` 中主动查询并向模型注入本会话最近已记录的 `recentNodes` 时间线；赋予模型 `UPDATE` 与 `APPEND` 双重动作决策能力；若当前对话是在继续深入或推进此前已记录的同一事件，模型输出 `UPDATE` 动作并指定目标 `targetNodeId`；待确认卡片直观对比“原事件”与“充实更新”，确认后就地更新原节点内容，防止冗余膨胀。
3. **主动暂停生成完整保留错误堆栈与 Key 记录（需求 3）**：
   - **痛点分析**：当模型遇到连接错误（如 429、504、超时或断网）时，用户点击暂停生成，界面仅显示冷冰冰的“回复已被暂停”，原本的模型报错信息和多 Key 轮询尝试记录完全丢失，无法定位故障。
   - **重构方案**：在 `ChatViewModel.stopGeneration()` 中捕获当前 `_reconnectStatus`、`_error` 以及 `currentKeyAttemptErrors`；如果产生过异常，在被暂停的消息正文末尾自动追加格式化专区 `【连接异常信息记录】`，包含异常状态、报错详情与多 Key 尝试记录，永久保留诊断信息。
4. **时间线梳理全程实时可见、草稿持久化与断点续梳（需求 4）**：
   - **痛点分析**：全量梳理长对话历史需要较长时间，过去无法实时查看中间提炼出的事件；且一旦退出或取消，已完成的梳理结果全部丢失，无法继续。
   - **重构方案**：
     - 新建 `TimelineDraftManager` 单例管理器，基于本地 JSON 文件实现会话级梳理草稿独立持久化；
     - `AiRepository.reconcileConversationTimeline` 引入分块流式回调 `onIntermediateResult`，每分析完一个分块即向 ViewModel 与磁盘草稿实时刷新；
     - `ChatScreen` 顶部新增实时梳理进度卡片，展示当前提炼事件数，并提供【实时查看】与【暂停/保存】；
     - 支持在未完成或取消后，直接从草稿中【查看草稿】或【接着进度继续向下梳理】，模型无缝从断点消息向后继续推演。
5. **长文本输入连接报错与空回复根治（需求 5）**：
   - **深层根因排查**：
     - 同步阻塞辅助调用：`ChatViewModel.sendMessageInternal` 中，`extractMemoryCandidate` 在流式对话发起前以同步阻塞方式 `execute()`，长文本输入越过正则门限触发调用，直接占满单并发端点或导致连接超时，拖垮主会话流式请求；
     - 推理模型参数冲突：OpenAI 思考模型（o1、o3、deepseek-r1、qwq 等）在传入自定义 `temperature` 时直接返回 HTTP 400 Bad Request；
     - 思考模型空回复崩溃：思考模型仅输出 `<thought>` 链而正文截断为空时，底层直接抛出未捕获 500 异常。
   - **重构方案**：
     - 将 `extractMemoryCandidate` 解耦移入 `applicationScope.launch(Dispatchers.IO)` 后台异步执行，与主对话流完全隔离；
     - 在 `requestTemperature`、滚动摘要、记忆提炼与时间线梳理中强制对推理模型指定 `temperature = null`；
     - 在流式接收器中增加思考完成但正文为空的保底回退提示 `*(思考已完成，但模型未输出正文内容...)*`，杜绝空回复崩溃。
6. **提炼设定两行式精致排版（需求 6）**：
   - **重构方案**：优化 `ChatStoryDialogs.kt` 中梳理时间线提取出的世界观与角色固有设定显示效果；采用两行式布局：第 1 行紧凑排布选择方框、设定性质徽章（世界规则/角色固有特征/行为约束等，点击可快速循环切换）、应用范围标签与删除按钮；第 2 行提供独立文字编辑框，编辑体验宽敞舒适。
7. **界面防挤压防错位与纯净视觉（需求 7）**：
   - **重构方案**：
     - 为时间线工作台中的【全不选】文字按钮添加 `weight(1f, fill = false)` 与 `softWrap = false`，杜绝窄屏下被挤压换行或错位；
     - 为【会话专属设定与规则】标题与总开关添加弹性权重约束，杜绝开关被挤出屏幕边缘；
     - 移除【会话专属设定与规则】标题前面的多余灯泡 `💡` 图案，视觉统一符合 Echo 液态玻璃规范。
8. **设定确认保存即刻入库生效强化（需求 8）**：
   - **痛点分析**：过去在时间线工作台保存提取出的世界观与固有设定时，若保存为全局无会话 ID，在普通会话专属记忆与角色扮演独立记忆库中均不可见，无法在后续对话中作为设定生效。
   - **重构方案**：在 `applyReconciledTimeline` 中，将用户确认的设定以 `conversationId` 保存进 `sessionMemories`，同时自动开启 `enableSessionMemory = true`；若当前会话为故事创作/角色扮演，同步镜像保存为固定的 `RoleplayMemory` 核心事实，确保在普通对话与角色扮演对话中均能 100% 立即生效。
