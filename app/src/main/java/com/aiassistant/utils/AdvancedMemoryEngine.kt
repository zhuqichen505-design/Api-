...
object AdvancedMemoryEngine {
    ...
    /**
     * 生成引导模型输出结构化多维摘要的标准系统提示词（强制保留时空演变与关键时间节点，杜绝时间错乱）
     */
    fun buildStructuredSummaryPrompt(
        ...
        return """
            ...
            严格按以下四部分输出，保证高信噪比与精准的时序锚定，杜绝时空错乱与废话流式记录：

            【核心背景与用户固定约束】
            - 列出长期不变的目标、底线规则、输出格式约束、关键角色设定与人际关系事实。

            【时空演变与关键时间节点（极重要，严禁遗漏）】
            - 起始时间与总跨度：注明故事开端时间（如第 1 天·夜晚），以及至当前停顿处累计跨越了多少天/时间跨度。
            - 重大里程碑时间锚点：注明核心事件发生在哪一天（如：[第 1 天·夜晚] 男女主角确立恋爱关系；[第 3 天·下午] 车站事件）。严禁将早期事件模糊为“昨天”！
            - 当前故事停顿节点：标明当前停留在的确切时间与时段（如：第 5 天·上午）。
            ...
            3. 时间概念必须严密准确，严禁出现前序事件时序倒流或将数天前事件混淆为昨天的错误。
